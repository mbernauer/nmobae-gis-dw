...
function main () {
	...
	extract_geometry.py ${YEAR}_STATE.zip STATE ${YEAR} | perl -F'\t' -lane 'print if $F[2] =~ /^35/' 
	extract_geometry.py ${YEAR}_COUNTY.zip COUNTY ${YEAR} | perl -F'\t' -lane 'print if $F[2] =~ /^35/' 
	extract_geometry.py ${YEAR}_TRACT.zip TRACT ${YEAR} | perl -F'\t' -lane 'print if $F[2] =~ /^35/'
}
